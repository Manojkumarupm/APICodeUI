...
LoadUsersGrid(): void {
  ...
  AddNewUser(form:  NgForm): void {
    const Userdetails: Users = {UserId: 0,
...
LastNameSort():  void {
  ...
  IdSort():  void {
      this._service.GetAllUsers().subscribe(data => this.list = data.sort((a,  b) => {
        if (a.UserId < b.UserId) { return -1; } else if (a.UserId > b.UserId) { return 1; } else { return 0; }
      }));
  ...
  const Userdetails:  Users = {UserId:  this.UserId,
  FirstName:  this.FirstName,
  LastName:  this.LastName,
  EmpId:  this.EmpId};
  if (Userdetails.FirstName === undefined || Userdetails.LastName === undefined || Userdetails.EmpId === undefined ||
